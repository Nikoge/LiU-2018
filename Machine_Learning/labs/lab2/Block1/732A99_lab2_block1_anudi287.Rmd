...
rng=function(data, mle) {
  ...
  data1=data.frame(MET=state_data$MET, residual=compare_data$residual)
  n=length(compare_data$actual_value)
#generate new Price
...
f1=function(data1){
    model <- tree(data = data1, 
                EX~MET, 
                control = tree.control(nobs=NROW(data1), minsize = 8))
  
  model_purned <- prune.tree(model, k = 3)
  
  compare_data <- predict(model_purned, newdata = data1)
  
  
}

res=boot(state_data, statistic=f1, R=1000, mle=mle, ran.gen=rng, sim="parametric")
