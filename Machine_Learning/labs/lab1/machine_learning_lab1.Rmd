...
geom_bar(stat="identity") + ggtitle("Plot of MSE vs. Features by folds") + coord_flip() 

best_variables <- result %>% group_by(type, variables) %>% filter(type == "test") %>% 
  summarise(MSE = mean(mse, na.rm = TRUE)) %>% arrange(MSE) %>% select(variables) %>% slice(1) %>% as.vector()
...
table_cofe <- head(result, 10) %>% select(Channel18, Channel78, Channel84, Channel62, 
...
```
## 3.7 Use cross-validation to find the optimal LASSO model, report the optimal and how many variables were chosen by the model and make conclusions

```{r}
# #find the best lambda from our list via cross-validation
lambda_lasso <- seq(from = 0, to = 100, length.out = 101)
lasso_cv <- cv.glmnet(x,y, alpha=1, lambda = lambda_lasso, type.measure="mse")
plot(lasso_cv)
coef(lasso_cv, lambda = lasso_cv$lambda.min)

lowest_lambda = lasso_cv$lambda.min
...
ggplot(result_lasso, aes(x = Lambda, y = Cross_Mean_Error)) + geom_point() + ggtitle("Cross Validation Error vs. Lambda")
```

Analysis: The minimum value of lambda was 0, implies zero penalisation. The variables selected are: Mositure, Protein, Channel 100, Channel41 along with intercept.
