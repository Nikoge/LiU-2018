...
pacman::p_load(xlsx, glmnet, MASS, jtools, huxtable, ggplot2, 
               ggthemes, gridExtra, ROCR, broom, caret, e1071, kknn)
...
df <- swiss
for(i in 1:NCOL(df)){
combs <- as.data.frame(gtools::combinations(NCOL(df), r=i, v=colnames(df), repeats.allowed=FALSE))
combs <- tidyr::unite(combs, "V1", sep = ",")
...
df3 <- train[,1:j, drop=FALSE]
df4 <- cbind(df3, Y = train$Y)
df4 <- as.data.frame(df4)

df5 <- test[,1:j, drop=FALSE]
df6 <- cbind(df5, Y = test$Y)
df6 <- as.data.frame(df6)
    
model <- lm(Y~., data = df4)
predicted <- predict(model, df6)

RMSE <- sqrt(mean((predicted - df6$Y)^2))
data <- cbind(i,j, RMSE)
...
colnames(result) <- c("kfold", "last_column_index", "RMSE")
return(result)  
