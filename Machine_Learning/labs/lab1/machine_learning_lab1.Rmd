...
```{r}
#Creating Protein function (increasing degree)

final_data <- tecator_data

final_data$Protein_2_power <- (final_data$Protein)^2 
final_data$Protein_3_power <- (final_data$Protein)^3 
final_data$Protein_4_power <- (final_data$Protein)^4 
final_data$Protein_5_power <- (final_data$Protein)^5 
final_data$Protein_6_power <- (final_data$Protein)^6 
```
Analysis:
...
full.model1 <- lm(Fat ~., data=tecator_data[,-1])
step.model1 <- stepAIC(min.model1, direction ='forward', scope=biggest1)
```
