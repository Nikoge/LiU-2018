...
st_new$h_date <- as.numeric(abs(difftime(st_new$new_date, st_new$date, units = c("days"))))

st_new$h_time <- as.numeric(abs(difftime(strptime(paste(st_new$new_date, st_new$new_time),"%Y-%m-%d %H:%M:%S"),
...
h2 <- 0.01
h3 <- 0.00001
st_new$h_distance_kernel <- exp(-(st_new$h_distance/h1)^2)
st_new$h_date_kernel <- exp(-(st_new$h_date/h2)^2)
st_new$h_time_kernel <- exp(-(st_new$h_time/h3)^2)
...
temperature_function <- function(df, date, b,a){
b <- 
a <-  
df <- st  
...
place_dist <-  geosphere::distHaversine(p1 = c(b,a), p2 = df[,c("longitude", "latitude")],r=6378137) 
days_dist <- as.numeric(abs(difftime(with(df, as.POSIXct(paste(date, time))), date, units = c("days"))))
hour_dist <- as.numeric(abs(difftime(strptime(date, df$new_time),"%Y-%m-%d %H:%M:%S"),
                          strptime(paste(date, df$time),"%Y-%m-%d %H:%M:%S"),
                          units = c("hour"))))
