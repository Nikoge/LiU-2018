...
pacman::p_load(geosphere, kernlab, neuralnet, geosphere, data.table)
...
st_new$h_date <- difftime(st_new$new_date, st_new$date, units = c("days"))
...
st_new$hd_dist <- difftime(st_new$new_date_time, 
                           as.POSIXct(paste(st_new$date, st_new$time), format="%Y-%m-%d %H:%M:%S"),
                          units = c("hour"))

## removing any negative dates and time
st_new$posterior_flag <- ifelse(st_new$h_distance > 0 & st_new$hd_dist > 0, "retain", "drop")
