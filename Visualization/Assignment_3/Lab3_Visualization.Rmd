...
rds$NAME_2 <- as.character(rds$NAME_2)

rds <- inner_join(x = rds[,c("NAME_1", "NAME_2", "TYPE_2", "geometry")], y = income_data_recasted, by=c("NAME_2" = "region"))
class(rds) <- "sf"

#plotly
plot_ly() %>% add_sf(data=rds, split=~NAME_2, color=~Young, showlegend=F, alpha=1)
plot_ly() %>% add_sf(data=rds, split=~NAME_2, color=~Adult, showlegend=F, alpha=1)
